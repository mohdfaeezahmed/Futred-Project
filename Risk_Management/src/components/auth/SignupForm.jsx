...
const navigate = useNavigate();
  ...
  try {
    ...
    const response = await fetch('https://backend-latv.vercel.app/api/auth/login', {
      method: 'POST',
      headers: {
  ...
  <InputField
    ...
    name="email"
    error={errors.email}
    className="bg-transparent border border-gray-600 focus:border-blue-500 text-white placeholder-gray-400" 
  />
  ...
  {show && <div className='text-red-500'>Wrong Credentials</div>}
...
const SignupForm = ({ onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://backend-latv.vercel.app/api/auth/signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    const data = await response.json();
      ...
      if (error.response) {
        setErrors({
          ...errors,
          apiError: error.response.data.message || 'Signup failed. Please try again.'
        });
